...
## This model uses transposed convolution as an upsampling technique
...
class Concatenate_segment(nn.Module): # concatenate feature maps produced by the encoding pathway and the denoising decoding pathway
    ...
    def forward(self, layer_1, layer_2, layer_3):
        x = torch.cat((layer_1, layer_2, layer_3), 1)
...
class DenoisingBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.up = get_up_layer(self.in_channels, self.out_channels_denoise, kernel_size=2, stride=2, dim=self.dim,
...
class SegmentationBlock(nn.Module):
    ...
    def forward(self, encoder_layer, denoising_layer, segmentation_layer, i):
        ...
        if self.normalization:
                up_layer = self.norm0(up_layer)  # normalization 0
        merged_layer = self.concat(up_layer, cropped_encoder_layer, cropped_denoising_layer)  # concatenation
        y = self.conv1(merged_layer)  # convolution 1
        y = self.act1(y)  # activation 1
        if self.normalization:
            y = self.norm1(y)  # normalization 1
